...
async function logEvent(_userId: string, event_type: string, meta: Record<string, unknown> = {}) {
  // userId is no longer needed here — the server derives it from the
  // verified JWT (see requireAuth in auth-middleware.server.ts).
  try {
    await logActivityEvent({ data: { event_type, meta } });
  } catch {}
}
