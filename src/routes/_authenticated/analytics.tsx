...
import { listActivityEvents } from "@/lib/data.functions";
...
function AnalyticsPage() {
  ...
  useEffect(() => {
    (async () => {
      // Same pattern as dashboard/sleep: verify the session is still valid
      // before hitting protected data endpoints, and bail out to /auth
      // instead of letting an expired-token error crash the page.
      const me = await getMe().catch(() => null);
      if (!me) { clearSession(); navigate({ to: "/auth" }); return; }
      try {
        const data = await listActivityEvents({ data: {} });
        setEvents(data ?? []);
      } catch {
        setLoadFailed(true);
      }
  ...
  const last7 = Array.from({ length: 7 }).map((_, i) => {
    ...
    const dayEvents = events.filter((e) => String(e.created_at).startsWith(key));
    return {
      day: label,
      ...
      <main className="mx-auto max-w-7xl px-6 pb-16 space-y-5">
        {loadFailed && (
          <div className="glass rounded-xl p-3 text-sm text-muted-foreground">
            Couldn't load recent activity history — showing today's live session stats only.
          </div>
        )}
