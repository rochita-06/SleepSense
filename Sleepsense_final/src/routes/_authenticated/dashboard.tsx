...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (p) {
        setName(p.name || "");
        setAgentName((p as { agent_name?: string }).agent_name || "Nova");
  ...
  const completeOnboarding = async (n: string, a: string, v: Voice) => {
    await supabase.from("profiles").upsert({ id: userId, name: n, agent_name: a, voice_gender: v, onboarded: true });
  ...
  const signOut = async () => { resetSession(); await supabase.auth.signOut(); navigate({ to: "/" }); };
  const deleteAccount = async () => {
    if (!confirm("Delete your account permanently? This cannot be undone.")) return;
    try { await deleteMyAccount(); await supabase.auth.signOut(); toast.success("Account deleted"); navigate({ to: "/" }); }
  ...
  const logEvent = async (event_type: string) => {
    await supabase.from("activity_events").insert({ user_id: userId, event_type });
    setRefresh((n) => n + 1);
  };
          ...
          <div className="space-y-5">
            <div className="glass rounded-2xl p-5">
              ...
              <input value={agentName} onChange={(e) => setAgentName(e.target.value)}
                onBlur={async () => { await supabase.from("profiles").update({ agent_name: agentName }).eq("id", userId); toast.success("Agent renamed"); }}
              ...
              <div className="mt-1 grid grid-cols-2 gap-2 text-xs">
                <button onClick={async () => {
                  setVoice("female");
                  await supabase.from("profiles").update({ voice_gender: "female" }).eq("id", userId);
                ...
                <button onClick={async () => {
                  setVoice("male");
                  await supabase.from("profiles").update({ voice_gender: "male" }).eq("id", userId);
